...
public class OpenCase extends AppCompatActivity
    ...
    LinearLayout.LayoutParams layoutParam = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT,1f);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        layoutParam.setMargins(toolBox._pixelSize*5,toolBox._pixelSize*5,toolBox._pixelSize*5,toolBox._pixelSize*5);
    }
    ...
    public LinearLayout  addItemList(String ID, String Weapon, String Skin, int Color, int Special){
        try{
            ...
            if(ID == "special"){
                _skin.setImageBitmap(toolBox.getBitmap("special/special"+Special+".png",80*toolBox._pixelSize, 40*toolBox._pixelSize));
            }else{
                _skin.setImageBitmap(toolBox.getBitmap("item/"+ID+".png", 80*toolBox._pixelSize, 40*toolBox._pixelSize));
            }

            TextView _weaponName = new TextView(getApplicationContext());
            _weaponName.setTypeface(_font);
            _weaponName.setText(Weapon);
            _weaponName.setTextSize(7);
            ...
            _skinName.setTextSize(7);
            _skinName.setBackgroundColor(toolBox.getColorValue(Color));
            _skinName.setTextColor(getResources().getColor(R.color.textcolor));

            if(_weaponName.length() > 14){
                _weaponName.setTextSize(6);
            }

            _child.addView(_skin);
            _skin.setBackgroundResource(R.mipmap.gun_background);
            _skin.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 40*pixels));

            _child.setLayoutParams(layoutParam);
    ...
    public void listingCases(){
        ...
        try {
            ...
            while (_cr.moveToNext()){
                ...
                TextView _oCaseName = toolBox.customTextView(_cr.getString(_cr.getColumnIndex("Name")),-1,8,true);
                _oCaseName.setTypeface(_font);

                _oSkin.setImageBitmap(toolBox.getBitmap("case/case"+_id+".png", 56, 40));
            ...
            while (add != 0){
                _father.addView(toolBox.blankItem());
                add--;
            }
   ...
   List<Integer> _itemList = new ArrayList<Integer>();
    ...
    public void openCase(){
        _db.setStatic("CASE_OPENING",1);
        _db.setStatic("CASE_"+_caseId, 1);
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                int intItemColor = Integer.parseInt(_db.getItemInfo(_winItemId,"Color"));

                if(!_db.checkStattrak(_winItemId)){
                    _db.setStatic("CASE_WIN_SPECIAL",1);
                    _winItemStattrak = 0;
                ...
                if(_finalStattrak != 0){
                    _db.setStatic("CASE_WIN_STATTRAK_"+intItemColor,1);
                }

                _db.setStatic("CASE_WIN_"+intItemColor,1);

                toolBox.playSound(R.raw.caseopen);

                int intXP = (_finalStattrak * (intItemColor * 50))+(intItemColor * 50);
              ...
              _objectOpenScrool.setVisibility(View.INVISIBLE);
                ...
                _scroolClass._child.removeAllViews();
            }
        },7000);
    }
