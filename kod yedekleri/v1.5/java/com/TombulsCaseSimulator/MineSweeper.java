...
public class MineSweeper extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intMaxGain = _db.getStatic("MINE_MAX_GAIN");
        intMaxPot = _db.getStatic("MINE_MAX_POT");
    }
    ...
    public void mineLose(){
        ...
        _db.setStatic("MINE_LOSE",1);
        _db.setStatic("MINE_POT_LOSE",_playValue);
    ...
    public void mineFinish(){
        ...
        _db.setStatic("MINE_GAME_MODE_X"+_mineCount,1);
        ...
        if (intMaxPot < _playValue){
            _db.setStatic("MINE_MAX_POT", _playValue);
            intMaxPot = _playValue;
        }

        if(winorLose){
            if(intMaxGain < _totalPrize + _playValue){
                _db.setStatic("MINE_MAX_GAIN", (_totalPrize + _playValue));
                intMaxGain = _totalPrize + _playValue;
            }

            _db.setStatic("MINE_POT_WIN", (_totalPrize + _playValue));
            _db.setStatic("MINE_WIN",1);
